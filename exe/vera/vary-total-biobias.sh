...
#SBATCH --job-name=vary-total-biobias
...
DATA_DIR="$MIMER_PATH/experiments/vary-num-total/biobias/data_deepseek"
...
for n in "${num_expert[@]}"; do
    ...
    python3 "$BASE_DIR/lib/vary_total_realworld.py" \
        "$n" \
	    "$MIMER_PATH/annotations/biobias/annotated_deepseek.json" \
        "$DATA_DIR_N/data_biobias_${SLURM_ARRAY_TASK_ID}.npz" \
